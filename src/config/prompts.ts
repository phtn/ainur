...
- Use crawl_codebase when you need AST-aware identifier lookup across the workspace or a git repository; it finds imports, referenced identifiers, and JSX usage.
...
export function getActiveSystemPrompt(): string {
  ...
  const base = data.active && data.presets[data.active] ? data.presets[data.active]! : DEFAULT_SYSTEM_PROMPT
...
export function getPrimaryRolePrompt(): string {
  ...
  const parts = roleFiles.map((file) => readRoleFile(file)).filter((value): value is string => Boolean(value))
  return parts.join('\n\n').trim()
}

function getDefaultRoleFiles(): string[] {
  const root = getWorkspaceRoot()
  return [join(root, 'SOUL.md'), join(root, 'USER.md'), join(root, 'AGENTS.md'), join(root, 'MEMORY.md')].filter(
    (path) => existsSync(path)
  )
}

function readRoleFile(filePath: string): string | null {
  const expanded = filePath.startsWith('~') ? resolve(homedir(), filePath.slice(1)) : resolve(filePath)
...
export function listPresets(): ListPresetsResult {
  const data = loadPromptsRaw()
  const active = data.active && data.presets[data.active] ? data.active : null
